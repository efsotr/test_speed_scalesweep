"""Editable NVFP4 ScaleSweep kernels for latency experiments.

This file vendors the two ``scalesweep*_nvfp4_utils.py`` implementations.
FP4 tensor allocation and the CUTLASS fallback reuse vLLM's ``custom_ops``.
Edit the kernels here, then call ``scaled_fp4_quant(..., backend=...)``.
"""
...
def scaled_fp4_quant(
    ...
    """Dispatch to the vendored ScaleSweep variants or vLLM CUTLASS."""
    if input.ndim < 1:
        raise ValueError("input must have at least one dimension")
    input = input.reshape(1 if input.ndim == 1 else -1, input.shape[-1])
    if input.shape[-1] % BLOCK_SIZE:
        raise ValueError("the last input dimension must be a multiple of 16")
    if backend == "scalesweep":
        return scalesweep_nvfp4_quant_impl(
            input, input_global_scale, is_sf_swizzled_layout, padded_n
        )
    if backend == "scalesweep128":
        return scalesweep128_nvfp4_quant_impl(
            input, input_global_scale, is_sf_swizzled_layout, padded_n
        )
    if backend == "scalesweep_mse":
        return scalesweep_mse_nvfp4_quant_impl(
            input, input_global_scale, is_sf_swizzled_layout, padded_n
        )
    if backend == "scalesweep_mse128":
        return scalesweep_mse128_nvfp4_quant_impl(
            input, input_global_scale, is_sf_swizzled_layout, padded_n
        )
    if backend == "cutlass":
        return vllm_ops.scaled_fp4_quant(
            input, input_global_scale, is_sf_swizzled_layout,
            backend=backend, padded_n=padded_n,
        )
