...
try:
    ...
    from .scalesweep_mse_nvfp4_utils import (
        BLOCK_SIZE,
        LOWER_BOUND,
        REF_MAX_SCALE_RAW,
        UPPER_BOUND,
...
def _fp32x16_e2m1_quant_weighted_squared_error(
    ...
    e0 = _fp32x2_e2m1_quant_weighted_squared_error(
        v0 * inv_scale, v1 * inv_scale, iw0, iw1
    )
    e1 = _fp32x2_e2m1_quant_weighted_squared_error(
        v2 * inv_scale, v3 * inv_scale, iw2, iw3
    )
    e2 = _fp32x2_e2m1_quant_weighted_squared_error(
        v4 * inv_scale, v5 * inv_scale, iw4, iw5
    )
    e3 = _fp32x2_e2m1_quant_weighted_squared_error(
        v6 * inv_scale, v7 * inv_scale, iw6, iw7
    )
    e4 = _fp32x2_e2m1_quant_weighted_squared_error(
        v8 * inv_scale, v9 * inv_scale, iw8, iw9
    )
    e5 = _fp32x2_e2m1_quant_weighted_squared_error(
        v10 * inv_scale, v11 * inv_scale, iw10, iw11
    )
    e6 = _fp32x2_e2m1_quant_weighted_squared_error(
        v12 * inv_scale, v13 * inv_scale, iw12, iw13
    )
    e7 = _fp32x2_e2m1_quant_weighted_squared_error(
        v14 * inv_scale, v15 * inv_scale, iw14, iw15
    )
    return (((e0 + e1) + (e2 + e3)) + ((e4 + e5) + (e6 + e7))) * (scale * scale)
