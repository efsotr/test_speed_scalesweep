...
def ensure_backends_available(backends: Sequence[str]) -> dict[str, bool]:
    """Import and verify registration of every requested vLLM quantization op."""
    if any(backend in SCALESWEEP_BACKENDS for backend in backends):
        # Registration is performed as an import side effect. The non-MSE module
        # imports the MSE module too, so this covers all four ScaleSweep ops.
        from vllm.model_executor.layers.quantization.utils import (  # noqa: F401
            scalesweep_nvfp4_utils,
        )
    ...
    for backend in backends:
        ...
        exists = hasattr(namespace, op_name) and hasattr(
            getattr(namespace, op_name), "out"
        )
...
def measure_latency(
    ...
    quantized, block_scales = ops.scaled_fp4_quant(
        activations,
